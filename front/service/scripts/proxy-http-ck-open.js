...
module.exports = async (ctx, src, packet, listener) => {
    ...
    if (listener.interface !== 'http') {
        log.info(`${lhd} << failed proxy http for ck open. not supported interface. exepected [http], actual [${listener.interface}]`);
        return utils.makeResData('E0001', 'Not supported interface');
    }

    if (!listener.req.get('h-authorization')) {
        log.info(`${lhd} << failed proxy http for ck open. not found mandatory header. key [h-authorization]`);
    ...
    if (splitAuthHeader.length !== 2) {
        log.info(`${lhd} << failed proxy http for ck open. invalid h-authorization. expected [h-authorization: {{apiType}} {{authData}}], actual [${listener.req.get('h-authorization')}]`);
        return utils.makeResData('E0001', 'Wrong Request');
    }

    // api type 이 안맞으면 에러
    if (splitAuthHeader[0] !== sValues.API_TYPE_OPEN) {
        log.warn(`${lhd} << failed proxy http for ck open. invalid api type. expected [${sValues.API_TYPE_OPEN}], actual [${splitAuthHeader[0]}]`);
        return utils.makeResData('E0001', 'Wrong Request');
    }

    const splitAuthData = splitAuthHeader[1].split(':');
    if (splitAuthData.length !== 2) {
        log.info(`${lhd} << failed proxy http for ck open. invalid h-authorization. expected [h-authorization: {{apiType}} {{accessToken}}:{{signature}}], actual [${listener.req.get('h-authorization')}]`);
        return utils.makeResData('E0001', 'Wrong Request');
    }
    ...
    log.debug(`${lhd} check api type [${apiType}], api key [${apiKey}], signature [${signature}]`);
    ...
    if (!findAppInfo.succ) {
        log.error(`${lhd} << failed proxy http for ck open. failed find app by api key. err [${findAppInfo.err}]`);
        return utils.makeResData('E0001', 'Failed');
    }
    app = findAppInfo.data;
    
    if (!app) {
        log.warn(`${lhd} << failed proxy http for ck open. not found app. api key [${apiKey}]`);
        return utils.makeResData('E0001', 'Wrong Request');
    }

    // check signature
    const isSignatureCorrect = await modules.core.crypto.checkSignature(signature, req.method, body, hDate, req._parsedUrl.pathname, app.api_secret, hContentType, lhd);
